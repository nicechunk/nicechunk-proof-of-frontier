...
nicechunk-proof-of-frontier is a focused split from the NiceChunk working tree. Public split repositories must not contain private keys, wallet files, deployment-only scripts, machine-specific configuration, production tokens, server addresses, local debug material, or generated build artifacts.
